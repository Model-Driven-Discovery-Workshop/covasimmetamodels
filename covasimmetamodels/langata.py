...
def run_model(parameters, pop_size=2e5):
    ...
    assert pars["n_days"] == len(betas)*parameters["beta_window"], "Mismatch of number of days of simulation and interventions (%s ~= %s*%s)"%(pars["n_days"], len(betas), parameters["beta_window"])

    dates = [i*parameters["beta_window"] for i in range( len(betas) )] #day when the beta values change
    pars["interventions"] = cv.change_beta(dates, betas)
    pars["use_waning"]=True #permit reinfection
